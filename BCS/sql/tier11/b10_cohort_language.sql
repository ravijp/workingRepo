-- Tier 11 | Cohort language: what the January callers say, and what it is worth
-- One transcript scan over the DQ1 cohort's January episodes (bridge classes
-- a, b, c). Each episode goes to its FIRST matching language group, priority:
-- future-dated promise (h6 lexicon), payment talk without promise or plan
-- (f1 loose lexicon), plan/settlement talk (v9 lexicon), hardship talk (v9
-- lexicon), dispute/fraud talk (h5 lexicon), none. Per group x class:
-- episodes, accounts, capture rate (f1's clean-payment gate), and 12-month
-- charge-off rate on distinct accounts (forward chrgoff_dt scan, END_12M =
-- 202512). Episodes without a transcript land in group f.
WITH snap AS (
