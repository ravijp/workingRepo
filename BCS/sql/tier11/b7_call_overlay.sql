-- Tier 11 | Call overlay: who called inbound in January, by bridge class
-- The month-max lens payoff. Customers call on intra-month delinquency, not
-- on where the account lands at month-end. Class a is the population ASP
-- month-end never sees (delinquent during January, current again by EOM) -
-- if its call rate is material, the month-max companion lens is justified
-- for the call story. Call side copies f1's convention: inbound legs,
-- business-card excluded, episode = first inbound leg per account per day.
-- Pre-2025 charge-off stock (202501 co_dt before 2025-01-01) is excluded
-- from all classes.
WITH snap AS (
