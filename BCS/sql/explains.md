...
Window: same cohort episode base as b10; one transcript scan for first-mention timing and agent offers, one customer-only scan for the first-third read. Why: whether the intent signal is operationally usable - visible early enough for a live read (n5's first-third logic), raised by the customer or pivoted by the agent (h2's min-beginmillis logic), and met by an agent plan/assistance offer (n4's lexicons). How to read: signals 1-3 partition the episodes; 4-5 cover intent episodes only; 6-7 cover customer-intent episodes only (n4's narrower pay lexicon), so the three blocks have different denominators - compare capture rates within a block, per class. The 6 vs 7 capture gap is the recoverable-miss read on the cohort. Caveats: episodes without a transcript count under signal 3 only; first-third is over the CUSTOMER's utterances, not elapsed time; association, not causation - agents may offer more when capture already looks likely.
