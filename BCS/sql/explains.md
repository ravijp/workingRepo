...
Window: account side 202412-202501 snapshots; call side January 2025 inbound episodes; payment gate reads January and February payment dates; charge-off from a 2025 forward scan of chrgoff_dt. Why: the within-January funnel for the DQ1 cohort itself, per bridge class - whether the leak-to-charge-off story differs for the ASP-invisible cured entrants (class a), the entrants still DQ1 at month-end (class b), and the EOM stock (class c). Class d is dropped; the story is the DQ1 cohort, not all delinquents. How to read: stages copy f1 - episode, transcript, customer payment/plan language (loose lexicon), no clean payment within 30 days (autopay- and NSF-dated payments do not count), charged off within 8 months. Stage 1 episodes are 0 by construction; compare stage shares across classes, not levels. Caveats: transcripts and payment gates are scoped to the cohort's January episodes only; the acctid fill gap on calls carries through every stage; the 8-month window uses the forward scan because January snapshots cannot carry future charge-off dates.

## b9_cohort_outcomes

Window: account side 202412-202501 snapshots; caller status from January 2025 inbound episodes with f1's clean-payment gate; charge-off windows END_8M=202508, END_10M=202510, END_12M=202512 from the forward chrgoff_dt scan. Why: the outcome table for the cohort - charge-off at 8, 10, and 12 months by bridge class and January caller status, the analogue of Ishant's CO_8M 38,087 (20.4%) / CO_10M 45,524 (24.4%) / CO_12M 51,326 (27.5%) on his 186,412 stock. How to read: our stock analogue is class b plus class c (the EOM side); compare shares, not counts - his base carries the ex-AA and CHRGOFF_RSN filters ours cannot mirror. Within a class, non-caller vs captured vs leaked is the value contrast. Caveats: caller splits are association, not causation - callers self-select; an account is 'captured' if ANY January episode passes the gate; charge-offs are counted from co_dt_future, so an account whose charge-off never surfaces in a 2025 snapshot is missed.

## b10_cohort_language

Window: the DQ1 cohort's January 2025 inbound episodes (bridge classes a, b, c from the cohort bridge; class d dropped); transcripts read at effdt 2025-01; charge-off from the 2025 forward chrgoff_dt scan (END_12M = 202512). Why: what the cohort's callers actually say, and what each language group is worth - the transcript-depth read for the January story. How to read: each episode goes to its FIRST matching group, priority: future-dated promise (h6 lexicon), payment talk without promise or plan (f1 loose lexicon), plan/settlement (v9 lexicon), hardship (v9), dispute/fraud (h5), none. pct_captured is the episode share passing f1's clean-payment gate; pct_co_12m is over the row's DISTINCT accounts. Caveats: groups b and c split the loose payment lexicon - plan/settlement terms are a subset of it, so 'payment talk' here means pay talk with no plan terms; episodes without a transcript land in group f; lexicon hits are string matches on ASR text, not verified promises.

## b11_cohort_signal_timing

Window: same cohort episode base as b10; ONE transcript scan, aggregates only (the first build used a second customer-only pass with ntile() and exhausted cluster memory at 618GB scanned). Why: whether the intent signal is operationally usable - visible early enough for a live read (n5's first-third logic), raised by the customer or pivoted by the agent (h2's min-beginmillis logic), and met by an agent plan/assistance offer (n4's lexicons). How to read: signals 1-3 partition the episodes; 4-5 cover intent episodes only; 6-7 cover customer-intent episodes only (n4's narrower pay lexicon), so the three blocks have different denominators - compare capture rates within a block, per class. The 6 vs 7 capture gap is the recoverable-miss read on the cohort. Caveats: episodes without a transcript count under signal 3 only; first-third is over elapsed call time (first intent timestamp within a third of the call's span), a time-based stand-in for n5's utterance terciles; association, not causation - agents may offer more when capture already looks likely.

## b12_cleanup_check

Window: December 2024 + January 2025 account snapshots only. Why: b7-b11 exclude accounts whose January rows carry a charge-off date before 2025-01-01 (old charged-off stock, mostly charge-off reversals and re-ages whose chrgoff_dt still holds the historical date); this makes that exclusion visible and auditable, and splits the 31,810 month-END entrants that are off the strict month-MAX path into their two causes (touched bucket 2+ inside January vs briefly past due inside December). How to read: b and e are the excluded accounts (expect 1,064 and 1,619 against ladder steps f and g); c and f reproduce the class-b and class-c denominators used in b7-b11 (151,113 and 53,210); h + i must sum to g. Caveats: the exclusion lives in OUR SQL (the classed CTE), not in the SAS side; chrgoff_dt is as recorded on January snapshots.

## b13_cycle_confound

Window: December 2024 + January 2025 account snapshots only (no forward scan). Why: the story's 69/31 cured-vs-still split is a calendar-month cut, and Anupam named the mechanics on 07-08 - reporting runs on EOM, collections runs on cycles, so a Jan-27 entrant had four days of in-month runway against a Jan-3 entrant's four weeks. The table is daily, so entry day (first January snapshot at bucket >= 1) is measured, not proxied. How to read: one row per entry day over the cleaned entrant cohort (b7-b11's classes a+b; entrants should sum to ~492,074); the pct-cured line against days-runway is the controlled read. If cure falls steadily with entry day, part of the 69/31 split is runway; compare early-month vs late-month bands before quoting the raw split as behavior. Caveats: entry day-of-month is a stand-in for cycle position (the cycle date itself is not in this table); snapshots cover ~29 dates in January, so a day with no snapshot shifts entry to the next snapshot day; cure here is bucket 0 at Jan 31, the same EOM definition as the ladder.

## m1_stilldq1_phrases_capture

Window: January 2025 cohort episodes, class b only (month-MAX B1 entrants still DQ1 at Jan 31, ~9,013 episodes), December+January account snapshots for the class, Jan-Feb payment fields for the capture gate. Why: n1 learned bigrams platform-wide; the hybrid lexicon upgrade needs phrases mined where the story's money is - the still-delinquent class - and needs trigrams as well as bigrams. Winners are lexicon CANDIDATES: nothing enters the story until it is re-measured at full count (lift vs capture and CO). How to read: side a = leak-markers (over-represented on episodes with no clean payment in 30 days), side b = capture-markers; lift is the ratio of episode shares; support >= 40 episodes, so small counts are already filtered. Caveats: ONE transcript pass, aggregates only (b11's memory lesson); phrases mark people as much as calls - association, not causation; the class is ~9k episodes, so trigram supports run low and the top lists are shorter than n1's platform-wide ones; all-filler phrases dropped with n1's stoplist.

## m2_stilldq1_phrases_co

Window: same class-b episode base as m1; outcome from a forward chrgoff_dt scan through 202512 (END_12M, mirroring the waterfall windows). Why: m1 ranks phrases against the 30-day capture gate, which partly restates payment mechanics; this companion ranks the same phrase space against the account's 12-month charge-off, the money outcome. A phrase high on m1's leak side AND this charge-off side is the strongest candidate for the live-read lexicon. How to read: side a = charge-off markers, side b = survival markers; lift is the ratio of episode shares between accounts that charged off within 12 months and those that did not. Caveats: one transcript pass, aggregates only; outcome is at ACCOUNT grain applied to every episode of that account; association only - hardship words do not cause charge-off, they mark it.

## b14_ledger_transitions

Window: December 2024 through February 2025 account snapshots; call and transcript side January 2025 inbound episodes (f1's convention); payment gate reads January and February payment fields. Grain: month-END on the ledger definition (Jan-31 EOM past-due bucket 1) and on the February position; the b12 cleanup (pre-2025 chrgoff_dt on January rows) is applied. Why: the ledger's February transitions at full count with the two confounds attached - who called (b9's class rule, with b9's leaked callers split by b8's intent lexicon into leaked-intent and other-caller) and how much in-month runway the account had (b13's entry-day logic; accounts past due at Dec-31 EOM are the carried-in band). The balance column is the CLEANED Jan-31 EOM ledger balance, a new number - b1's $537.4M is uncleaned. How to read: runway bands carry a path suffix (1 = strict month-MAX B1 entrant, 2 = other Dec-EOM-0 stock); sum the suffixes for the packet's four bands. Tie-outs the import report should check: total accounts 204,323 (151,113 entrant-path + 53,210 stock); caller classes 193,906 non-caller / 6,630 captured / 2,394 leaked-intent / 1,393 other-caller (the last is derived: 10,417 callers minus the two named classes - if b9's implemented precedence makes the actual differ, report the actual and flag it); suffix-1 runway bands must reproduce b13's still-DQ1 composition 43,227 / 51,859 / 56,027; cure/stay/roll shares within about 1pp of b6's ~54.9 / ~7.9 / ~36.8. Caveats: caller classes inherit the acctid fill gap on calls; the intent split needs a transcript, so no-transcript callers land in other-caller; one transcript pass, aggregates only.

## b15_leaklist_forward

Window: December 2024 through March 2025 account snapshots; January 2025 inbound episodes for the intent and deceased lexicons; charge-off from the 2025 forward chrgoff_dt scan (8M = 202508, 10M = 202510, 12M = 202512). Grain: account, on the month-END ledger (Jan-31 EOM bucket 1, b12 cleanup applied). Why: the work list followed forward - the leaked-intent accounts (b8's definition: no clean payment within 30 days on any January episode, at least one intent episode) tracked to Feb and Mar EOM and through the charge-off windows, with the deceased-estate flag kept as a COLUMN so both views render rather than removing the estate calls. How to read: Feb/Mar positions follow b6's convention ('co' = charge-off date lands in the window, 'gone' = no row that month); tie-outs: accounts must sum to 2,394 and b15_co_8m to 880; the 12-month charge-off on this list is a NEW number - quote it as such. Caveats: the deceased flag is a lexicon string-match on ASR text, not a verified estate status; one transcript pass, aggregates only; CO windows read from co_dt_future, so a charge-off never surfacing in a 2025 snapshot is missed.

## b16_within_cohort_contrast

SUPERSEDED by b16_v2: scoring every January episode against the account's one Feb-EOM position made both outcome rows identical, and gating the paid side on transcript + intent language cut the both-kinds population to 168 accounts against a 217 floor. Kept for the record; do not quote its numbers. Original entry follows. Window: December 2024 through February 2025 account snapshots; January 2025 inbound intent episodes; payment gate reads January and February payment fields. Grain: episode within account, on the month-END ledger (Jan-31 EOM bucket 1, b12 cleanup applied). Why: the cohort-native version of x4, the causal guard - only accounts with BOTH a paid-30d and a no-pay-30d January intent episode, so the who-calls selection effect cancels within the account. How to read: compare the Feb-position mix of paid vs no-pay episodes; both rows share the same accounts by construction. Cells may be thin - report the sizes regardless; if thin, x4 (funnel-wide, 31,804 accounts) remains the labeled guard. Caveats: the capture gate here is the cohort series' CLEAN gate (b8/b9: autopay- and NSF-dated payments do not count) where x4 itself used the raw gate - the two guards are not interchangeable; the packet's wording ('paid-30d and no-pay-30d episode') does not mention intent, but x4's implemented contrast runs on intent episodes and that is followed here; Feb position is the account's, identical across its episodes - the contrast lives in the episode counts, not in different outcomes per row.

## b16_v2_within_cohort_contrast

Window: January through April 2025 account snapshots (April supplies the next-month read for March episodes); January through March 2025 inbound episodes (f1's convention, business-card excluded); ledger membership from the Jan-31 EOM snapshot (bucket 1, b12 cleanup, per b14/b15). Grain: episode, each scored on ITS OWN next-month bucket move (month-MAX bucket, x4's grain), which fixes b16's flaw of scoring every January episode against the account's single Feb position. Why: the causal guard on the ledger itself - only accounts with BOTH a paid-30d and a no-pay-30d episode in the window, so the who-calls selection effect cancels within the account. No transcript pass: episode kind is the payment gate alone, so the paid side matches b14's captured rule and the January-only both-kinds population has a floor of 217 accounts (b8 2,394 minus b14 2,177); the Feb-Mar episodes add to that. How to read: if pct_current_next_month is visibly higher after paid episodes than after no-pay episodes FOR THE SAME ACCOUNTS, capture moves the account, not just one payment. Expect far fewer accounts than x4's funnel-wide 31,804 - the population is one month's DQ1 EOM ledger over a 3-month window, not all delinquents over 12 months. Caveats: the capture gate is the cohort series' CLEAN gate (b8/b9: autopay- and NSF-dated payments do not count) where x4 used the raw gate, and no intent language is required where x4 required it - so v2's numbers tie to b14's classes, not to x4's; episodes need the account delinquent (month-MAX >= 1) and not charged off in the call month, and a next-month snapshot; still not randomized - paid episodes may sit earlier in the spell.

## b17_callday_population

Window: account snapshots 2024-06-01 through January 2025 (daily grain); call side January 2025 inbound episodes (f1's convention, business-card excluded). Grain: as-of-call-day snapshot, not month-max and not month-end - the only b-series query on this grain. Why: sizes the operable call stream by what is knowable AT CALL TIME, and makes the bucket-1 scope choice explicit by counting the deeper-bucket episodes a bucket-1-scoped play would ignore. How to read: rows are bucket at call (1 / 2-6 / 7+) x days since the current spell's first delinquent snapshot (0-10 / 11-20 / 21+); the spell start is the first bucket >= 1 snapshot after the account's last bucket-0 snapshot at or before the call. Sanity expectation, not a hard tie-out: total episodes <= 122,606 (b7's delinquent-in-month sum); the shortfall is calls made before the account's first delinquent snapshot - intent arriving ahead of the ledger, itself worth noting. Caveats: the 2024-06 lookback floor caps the spell start (v11/b2's floor) - deep-bucket spells older than that band as 21+ anyway; pre-2025 chrgoff_dt stock is excluded off the call-day snapshot's chrgoff_dt, a slightly different read than b12's January-rows rule; a day with no snapshot shifts both the call-day bucket and the spell start to the previous/next snapshot day.

## b18_join_gap

Window: ALL January 2025 inbound call rows (leg grain, no product exclusion); account master any snapshot (sfx_nbr = 0) for the join; December 2024 through January 2025 daily snapshots for the call-day bucket. Grain: call row x week; the bucket flag is as-of-call-day. Why: bounds the verified-only join hole on the target population, weekly, so pilot-time joinability has a historical baseline - the pilot can only act on calls it can join, and this is what that gate cost in January. How to read: 'not verified-joined' = no acctid or an acctid that does not resolve to the master (f4_match_by_auth's logic); among joined, the bucket >= 1 flag marks the calls a delinquency play could actually see. Sanity expectation: the joined share should sit in the C47 band (~72-79% acctid fill, ~21% of inbound unjoinable). Caveats: date_trunc('week') weeks start Monday, so the first and last January weeks are partial; join status is not correctness - f4_match_by_auth shows the failed-auth skew inside the unjoined slice; no episode dedup here by design, so counts are legs, not conversations.

## m4_deceased_group

Window: the ledger's January 2025 inbound episodes (month-END classes b + c, expected 12,544 = 9,013 + 3,531); December-January account snapshots for the ledger; January-February payment fields for the capture gate; charge-off from the 2025 forward chrgoff_dt scan (8M = 202508, 12M = 202512). Grain: episode for the split and paid-30d rate, account for the charge-off and overlap columns. Why: decides the deceased-estate routing rule and the deceased share of caller charge-offs - the b10-style read with a deceased-estate lexicon (passed away, death certificate, executor, deceased, calling on behalf - phrases from the measured m2 list, customer utterances only) as the TOP-priority group above b10's partition (promise, payment talk, plan, hardship, dispute, none). How to read: groups must partition the 12,544 episodes exactly - the tie-out; a deceased group with near-zero paid-30d and high charge-off is a routing case, not a collections case; m4_leaked_intent_accounts is the overlap with the b15 work list - deceased accounts inside it should be routed out, which is why b15 carries the flag as a column instead of removing them. Caveats: ONE transcript pass, aggregates only (b11's memory lesson); 'calling on behalf' also matches non-estate third-party callers, so the group is an upper bound - audit a sample before setting the routing rule; lexicon hits are ASR string matches, association not verification.

## m3_transcript_pii_surface

Window: none - information_schema metadata only, no data rows scanned. Why: PCDS tables come in twins (*_HI_SNSTV with raw card numbers, PII-gated, vs *_NON_PRV masked and accessible - validated on V_REAGEEVENTS in the 07-03..08 chat). Before any transcript sample is exported for the Copilot read, we must know whether the Athena transcript surface has an equivalent non-sensitive twin and what else sits in the contact-center schema. How to read: patterns a/b are name-matched twins anywhere in the catalog; d/e enumerate transcript-named tables and the contact-center schema. If no non-sensitive transcript twin exists, the export design must add its own masking step and the governance ask changes accordingly. Caveats: name matching only - a twin with a different naming convention would not surface; running this does NOT authorize an export (A-066 governance answer required first).

## e1_leak_list_202501 (export, sql/exports/)

Window: January 2025 snapshots for ledger membership; January episodes + January-February payment fields for the labels; one January transcript pass. Grain: account. Why: the id-list deliverable for CQ-5 - the EXCLUSIVE-rule leak list (>= 1 intent-no-pay episode, no paid-30d episode anywhere in January) with the deceased-estate lexicon flag as a column, so the SAS side can price the work list with and without the estate routing. How to read: NOT a report query - it returns 2,177 account rows (210 with deceased_flag = 1), the sanctioned exception because the list itself is the deliverable; download to output\exports\ (never output\csv\) and hand to the SAS side inside the client environment. Tie-outs: 2,177 rows / 210 deceased, matching b15/m4 exactly; any other count means the logic drifted from b15 - stop and diagnose. Caveats: membership, gate, and lexicons are copied verbatim from the verified b15; the deceased flag is an ASR string match, not verified estate status.

## e2_caller_paid30d_list (export, sql/exports/)

Window: January 2025 snapshots for ledger membership; January episodes + January-February payment fields; one January transcript pass. Grain: account. Why: the id-list deliverable for CQ-7 - all of the cleaned ledger's January callers, each with its Athena paid-30d label, so PAYMT_AMT on the SAS side can audit the capture gate itself. How to read: NOT a report query - it returns 10,417 account rows labeled captured / leaked / other_caller; download to output\exports\ and hand to the SAS side inside the client environment. Tie-outs: 10,417 rows; captured 6,630; leaked 2,177; other_caller 1,610 (derived: 10,417 minus the two measured classes - if the actual differs, report the actual and flag it). Caveats: labels inherit the acctid fill gap (~28% of January inbound is unjoinable, b18) and the no-transcript callers land in other_caller, exactly as in b14.
