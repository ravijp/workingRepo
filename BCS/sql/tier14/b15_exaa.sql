-- Tier 14 | EX-AA VARIANT of b15_leaklist_forward -- NO EXCLUSION APPLIED.
-- Per the tier-14 brief: b15's leak-list population is UNCHANGED. Note on
-- the original's header: its "2,394 / 880" tie-outs are stale spec-era
-- values; the SQL's own gate (no clean payment within 30 days on ANY
-- January episode) is the EXCLUSIVE rule, and the verified run returned
-- 2,177 accounts / 210 deceased (bridge record round 7 + addendum 2).
-- This variant ADDS a class column (b15_pc_class, from clnt_prdct_cd at
-- the account's January EOM snapshot, eom_cpc = max_by(clnt_prdct_cd,
-- eff_dt)) as an extra grouping dimension on top of the existing
-- deceased-flag x Feb-position x Mar-position output. The ex-AA work list
-- is read off the 'd. others' rows; the AA/GM/Bronco rows name that share
-- of the leak list W. NULL-safe classification (NULL/blank cpc lands in
-- 'd. others', matching the athena-filter-check P-C query's CASE order).
-- Expected tie-outs (from the VERIFIED RUN, not the stale header):
-- b15_accounts across ALL class rows sums to 2,177 EXACTLY; deceased-flag
-- 'a' rows sum to 210 EXACTLY; b15_co_8m sums to 853 EXACTLY (148 deceased
-- + 705 others); b15_co_10m 943; b15_co_12m 1,006; balance ~9,752,424
-- (rounding tolerance). GM and Bronco rows expected at or near zero.
-- deceased flag x Feb x Mar cells are unchanged from the original run;
-- only the new class column is new measurement.
-- Everything else (population, CO windows, deceased lexicon, transcript
-- pass) is unchanged from b15_leaklist_forward.sql.
WITH snap AS (
