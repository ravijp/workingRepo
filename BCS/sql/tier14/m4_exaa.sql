-- Tier 14 | EX-AA VARIANT of m4_deceased_group (original lives in sql\tier12\).
...
-- SAS AA+GM+Bronco code set (23 codes). Applied on the ledger CTE (the
-- account population feeding the January episodes), so all m4 groups are
-- ex-AA. NULL-safe form: NULL or blank cpc is kept as "others".
-- Expected tie-out: m4_exaa's groups partition the ex-AA ledger's January
-- episode count exactly (internal identity, unchanged from the original);
-- the total <= the original m4's 12,544 episodes (ex-AA removes accounts,
-- so strictly fewer or equal episodes expected).
-- Everything else is unchanged from m4_deceased_group.sql: the bucket
-- ladder, the b12 cleanup rule, the payment gate, the deceased-estate
-- lexicon and priority order, the transcript pass, and the leaked-intent
-- overlap logic.
WITH snap AS (
