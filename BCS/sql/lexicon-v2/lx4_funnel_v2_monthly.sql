-- Tier 13 | Lexicon v2 on the funnel window: f2's monthly funnel plus the
-- deceased routing view and the execution-marker flag (W3: 2024-07..2025-06)
-- RUN ONLY AFTER lx1-lx3 pass their January tie-outs.
-- f2's chain, gates, windows, and eight output columns are kept VERBATIM:
-- same cumulative gates, same episode dedup, same RAW-payment 30d gate
-- (f1's headline additionally excludes autopay/NSF; f2 does not — so this
-- query's no_payment_30d matches the recorded f2 digest, NOT f1's 142,982).
-- Tie-out: the twelve recorded f2 digest rows must reproduce EXACTLY on
-- the first eight columns. The six lx4_ columns are new measurement:
--   lx4_deceased_eps           transcribed delinquent episodes with the
--                              deceased-estate lexicon (m4 five, verbatim)
--   lx4_pay_language_net_dec   pay-language episodes net of deceased
--   lx4_no_payment_30d_net_dec leaked episodes net of deceased (the
--                              routed funnel: estate calls are processing,
--                              not collectable leakage)
--   lx4_chargeoff_8m_net_dec   CO-8m leaked episodes net of deceased
--   lx4_exec_eps               transcribed delinquent episodes with
--                              execution language (lx2 flag, verbatim)
--   lx4_leaked_with_exec       leaked episodes WITH execution language
--                              (payment mechanics spoken, no payment seen:
--                              gate-false-negative candidates, e.g. mailed
--                              checks)
-- Grain: episode, delinquent-in-month (month-max), verified-joined only
-- (floors; ~28% January join hole per b18, ~21% recent months). 2024-07 is
-- a boundary artifact month (run record): quote nothing from that row.
WITH snap AS (
