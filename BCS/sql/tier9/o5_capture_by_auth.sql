-- Tier 9 | Capture by authentication outcome: the verification-block leakage cause
-- A delinquent caller who cannot get through verification cannot pay on the
-- call. This reads payment-after-call for delinquent callers by
-- authentication outcome - the operational half of the recoverable split:
-- failed-auth callers who still wanted to pay are process leakage, not
-- willingness leakage. Pairs with f4_match_by_auth (which measures who is
-- invisible); this measures what happens to the visible ones.
...
SELECT auth AS authenticationstatus,
